...
namespace SpiraProjectAddIn
{
    ...
    public partial class SpiraRibbon
    {
        ...
        void importer_OperationCompleted(object sender, ImportExportCompletedArgs e)
        {
            ...
            MessageBox.Show("Import/Export of " + e.RowCount.ToString() + " items successfully completed", "Import/Export Completed", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void btnImport_Click(object sender, RibbonControlEventArgs e)
        {
            ...
            DialogResult result = MessageBox.Show("Are you sure you want to Import data from Spira. This will overwrite any information in the project?", "Confirm Import", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            try
            {
                ...
                importer.ProjectApplication = Globals.ThisAddIn.Application;
        ...
        private void btnExport_Click(object sender, RibbonControlEventArgs e)
        {
            ...
            try
            {
                ...
                //Start the export process and attach the event handlers in case of error or completed
                importer = new Importer();
                importer.ProjectApplication = Globals.ThisAddIn.Application;
                importer.ProgressForm = this.progressForm;
                importer.ErrorOccurred += new Importer.ImportExportErrorHandler(importer_ErrorOccurred);
                importer.OperationCompleted += new Importer.ImportExportCompletedHandler(importer_OperationCompleted);
                importer.Export(projectId);
            }
            catch (System.Exception exception)
            {
        ...
        private void btnClear_Click(object sender, RibbonControlEventArgs e)
        {
            ...
            DialogResult result = MessageBox.Show("Are you sure you want to Clear the active project. This will remove all tasks, resources and assignments?", "Confirm Clear", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (result != DialogResult.Yes)
            {
                return;
            }
            Microsoft.Office.Interop.MSProject.Project project = (Microsoft.Office.Interop.MSProject.Project)Globals.ThisAddIn.Application.ActiveProject;
            if (project != null)
            {
                //Clear Tasks
                Microsoft.Office.Interop.MSProject.Tasks tasks = project.Tasks;
                ...
                {
                    for (int i = resources.Count; i > 0; i--)
                    {
                        resources[i].Delete();
                    }
                }
